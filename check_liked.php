...
function getTrackLikedStatus($trackId, $accessToken) {
    ...
    // Check liked songs via contains endpoint directly
    $containsResp = spotifyApiRequest('https://api.spotify.com/v1/me/library/contains?uris=spotify:track:' . $trackId, $accessToken, 'GET');
    if ($containsResp['code'] === 200) {
        $containsData = json_decode($containsResp['body'], true);
        if (is_array($containsData) && isset($containsData[0])) {
            $result['is_liked'] = (bool)$containsData[0];
        }
    }
...
$likeStatus = getTrackLikedStatus($trackId, $accessToken);
